...
class NodeHandler {
    ...
    async start(callback = () => {}) {
        if ( !await processhelper.checkProcessingRunning(this.options.binaryName)) {
            console.log(`Starting ${this.options.binaryName}...`)
            await processhelper.requestSpawn(
                `echo "\n" | ./${this.options.binaryName} --datadir ${this.options.dataPath} --syncmode "full" --ws --wsport ${this.options.wsport} --wsapi eth,web3 > /dev/null 2>output &`, callback, {
    ...
    async getStatus() {
        try {
            const isRunning = await processhelper.checkProcessingRunning(this.options.binaryName)
            let servicesRunning = await isPortReachable(this.options.wsport, { host: "localhost" })
            // if (servicesRunning) {
            //     try {
