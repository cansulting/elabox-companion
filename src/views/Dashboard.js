    ...
    API.latestBlock().then((responseJson) => {
      ...
      API.blockSizes().then((responseJson) => {
        ...
        let blockTime = new Intl.DateTimeFormat("en-US", {
          ...
          minute: "2-digit",
          second: "2-digit",
        }).format(new Date(responseJson.blockTime * 1000));
        let blockHash = responseJson.blockHash;
        let blockHashFormatted =
          blockHash.substring(0, 9) + "..." + blockHash.substring(54, 64);
        let blockMiner = responseJson.miner;
        // DID
        let blockSizeListDid = responseJson.blockSizeListDid;
        let blockTimeDid = new Intl.DateTimeFormat("en-US", {
          year: "numeric",
          month: "2-digit",
          day: "2-digit",
          hour: "2-digit",
          minute: "2-digit",
          second: "2-digit",
        }).format(new Date(responseJson.blockTimeDid * 1000));
        let blockHashDid = responseJson.blockHashDid;
        let blockHashFormattedDid =
          blockHashDid.substring(0, 9) + "..." + blockHashDid.substring(54, 64);
        let blockMinerDid = responseJson.minerDid;

        API.serviceStatus().then((responseJson) => {
          ...
          API.nbOfTx().then((responseJson) => {
            this.setState({
              blockheight: nodeinfo.result,
              blockheightDid: nodeinfoDid.result,
              blocktime: blockTime,
              blocktimeDid: blockTimeDid,
              blockSizeList: blockSizeList,
              blockSizeListDid: blockSizeListDid,
              blockhash: blockHashFormatted,
              blockhashDid: blockHashFormattedDid,
              miner: blockMiner,
              minerDid: blockMinerDid,
              latestSize: blockSizeList[9],
              latestSizeDid: blockSizeListDid[9],
              nbOfTxList: responseJson.nbOfTxList,
              latestNbOfTx: responseJson.nbOfTxList[9],
              nbOfTxListDid: responseJson.nbOfTxListDid,
              latestNbOfTxDid: responseJson.nbOfTxListDid[9],
              elaRunning,
              didRunning,
              carrierRunning,
              tokenRunning,
    ...
    return (
      ...
      >
        ...
        <CardGroup className="mb-4">
          <Widget04 icon="fa fa-gears" header={this.state.blockheight}>
            Best block
          </Widget04>
          <Widget04 icon="fa fa-clock-o" header={this.state.blocktime}>
            Block Time
          </Widget04>
          <Widget04 icon="fa fa-hashtag" header={this.state.blockhash}>
            Block Hash
          </Widget04>
          <Widget04 icon="fa fa-gears" header={this.state.miner}>
            MINER
          </Widget04>
        </CardGroup>
        ...
        <CardGroup className="mb-4">
          <Widget04 icon="fa fa-gears" header={this.state.blockheightDid}>
            Best block
          </Widget04>
          <Widget04 icon="fa fa-clock-o" header={this.state.blocktimeDid}>
            Block Time
          </Widget04>
          <Widget04 icon="fa fa-hashtag" header={this.state.blockhashDid}>
            Block Hash
          </Widget04>
          <Widget04 icon="fa fa-gears" header={this.state.minerDid}>
            MINER
          </Widget04>
        </CardGroup>
        ...
        <CardGroup className="mb-4">
          <Widget04 icon="fa fa-gears" header={this.state.carrierIp}>
            Carrier IP Address
          </Widget04>
        </CardGroup>
      </div>
